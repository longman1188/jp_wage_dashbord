...
# 2019年における都道府県別の平均賃金をヒートマップで作る

st.header('2019年：一人当たり平均賃金のヒートマップ')

jp_lat_lon = pd.read_csv('./pref_lat_lon.csv')
jp_lat_lon = jp_lat_lon.rename(columns={'pref_name': '都道府県名'}) # 列名を合わせる
# jp_lat_lon

df_pref_map = df_pref_ind[(df_pref_ind['年齢'] == '年齢計') & (df_pref_ind['集計年'] == 2019)]
df_pref_map = pd.merge(df_pref_map, jp_lat_lon, on='都道府県名') # jp_lat_lonを結合する
# 正規化　Y=(X-Xmin)/(Xmax-Xmin)　最小値0　最大値1
...
# 年齢階級別の全国一人当たり賃金：バブルチャート　+　plotlyでアニメーション
# X軸：一人当たり賃金　Y軸：　バブル：
